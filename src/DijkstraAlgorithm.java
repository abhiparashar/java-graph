...
public class DijkstraAlgorithm {
    ...
    public static int[] dijkstra(int nodes, List<List<List<Integer>>>graph, int src){
        ...
        while (!pq.isEmpty()){
            Pair rem = pq.remove();
            if(result[rem.vtx] != Integer.MAX_VALUE) continue;

            result[rem.vtx] = rem.wsf;
            for (List<Integer>e : graph.get(rem.vtx)){
                int nbr = e.get(0);
                int wt = e.get(1);
    ...
    public static void main(String[] args) {
        // Graph from the video: 7 vertices (0 to 6)
        int nodes = 7;
        int[][] edges = {
        ...
        List<List<List<Integer>>>graph = createGraph(nodes, edges);
